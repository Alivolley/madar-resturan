...
function EditOrderStatusModal({ show, onClose, detail, cardMutate }) {
   ...
   return (
      <Dialog open={show} onClose={closeModalHandler} fullWidth dir="rtl">
         <div className="relative p-5 pt-0 font-rokhRegular">
            ...
            <form onSubmit={handleSubmit(formSubmit)} className="mt-5 space-y-8">
               <div className="flex items-center justify-between gap-1">
                  <p className="text-sm text-textGray customMd:text-base">وضعیت قعلی :</p>
               ...
               <div className="flex items-center justify-between gap-3">
                  <p className="text-sm text-textGray customMd:text-base">تغییر وضعیت به :</p>
                     ...
                     <Controller
                        ...
                        render={({ field: { onChange, value }, fieldState }) => (
                           <FormControl error={!!errors?.orderStatus} fullWidth>
                              ...
                              >
                                 {detail?.status !== 'sending' && (
                                    <MenuItem className="!font-rokhRegular !text-sm" value="sending" dir="rtl">
                                       درحال ارسال
                                    </MenuItem>
                                 )}
                                 {detail?.status !== 'delivered' && (
                                    <MenuItem className="!font-rokhRegular !text-sm" value="delivered" dir="rtl">
                                       تحویل داده شده
                                    </MenuItem>
                                 )}

                                 {detail?.status !== 'unpaid' && (
                                    <MenuItem className="!font-rokhRegular !text-sm" value="unpaid" dir="rtl">
                                       پرداخت نشده
                                    </MenuItem>
                                 )}
                              </Select>
               ...
               <div className="flex flex-1 flex-col gap-1">
                  <p className="mb-2 text-sm text-[#626E94] customMd:text-base">دلیل تغییر وضعیت</p>
                  <TextField
                     fullWidth
                     multiline
                     rows={6}
